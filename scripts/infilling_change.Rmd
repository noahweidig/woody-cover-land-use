---
title: "Changes in woody cover within and between NLCD forest types in the eastern United States"
author: |
...
pacman::p_load(tidyverse, skimr, flextable, trend, sf, cowplot, colorspace, data.table, officer)
set_flextable_defaults(font.family = "Times New Roman", line.spacing = 1)

land_cover_class_01 <- tribble(
  ~class_01, ~class_name_01,
  41, "Deciduous Forest",
  42, "Evergreen Forest",
  43, "Mixed Forest")

land_cover_class_21 <- tribble(
  ~class_21, ~class_name_21,
  41, "Deciduous Forest",
  42, "Evergreen Forest",
  43, "Mixed Forest")
...
```{r import_data_ch}
area_ch <- fread("data/forest/nlcd_area_changed_forest.csv")
file_path_ch <- "data/forest/"
file_names_ch <- paste0("nlcd_rap_changed_forest_", 2001:2021, ".csv")
...
# Summarize all forests
type_total <- merged_ch %>%
  filter(year %in% c(2001, 2021)) %>%
  group_by(class_name_21, year) %>%
  summarise(area_wc = sum(area_wc),
            area_ha = sum(area_ha),
            .groups = 'drop') %>%
  mutate(perc_wc = (area_wc / area_ha) * 100) %>%
...
forest_total <- merged_ch %>%
  ...
  mutate(area_change = (area_wc_2021 - area_wc_2001),
         perc_change = (perc_wc_2021 - perc_wc_2001),
         perc_area_change = (area_change / area_wc_2001) * 100) %>%
  mutate(class_name_21 = "Total Forest", .before = area_wc_2001)

total <- rbind(forest_total, type_total) %>%
    dplyr::select(class_name_21, area_ha_2021, area_wc_2001, perc_wc_2001,
         area_wc_2021, perc_wc_2021, area_change, perc_area_change)

write_csv(total, "outputs/forest_change_no_type.csv")
...
lc_change_ch <- merged_ch %>%
  ...
  dplyr::select(change_class, area_ha_2021,
  ...
  mutate(across(where(is.numeric),
                ~ case_when(. >= 100 ~ round(., 0),
                            ...
                            . <= 1 & . >= -1 ~ round(., 3),
                            TRUE ~ round(., 0)))) %>%
  mutate(change_class = str_remove(str_remove(change_class, " Forest"),
                                   " Forest")) %>%
  write_csv("outputs/data/infilling_change_ft_l1.csv")
```
...
perc_plot <- 
  ...
  scale_fill_steps2(low = "#8E00A4",
                    mid = "white",
                    high = "#136600",
                    midpoint = 0, limits = c(0, 1.5), na.value = "grey",
                    name = "Sen's Slope\n(%/year)",
                    n.breaks = 6) +
...
ragg::agg_png(filename = "outputs/objective_2/infilling_perc_l1_fig.png", scaling = 0.9,
...
area_plot <- 
  ...
  scale_fill_steps2(low = "#8E00A4",
                    mid = "white",
                    high = "#136600",
                    midpoint = 0, limits = c(0, 50000), na.value = "grey",
                    name = "Sen's Slope\n(ha/year)",
...
# Save the combined plot
ragg::agg_png(filename = "outputs/objective_2/infilling_area_l1_fig.png",
              scaling = 0.9, height = 3, width = 5, units = "in", res = 600)
print(area_plot)
invisible(dev.off())
```
...
sen_data_l3 <- merged_ch %>%
  ...
  summarise(area_wc = as.numeric(sum(area_wc)),
            area_ha = as.numeric(sum(area_ha)), .groups = "drop") %>%
  write_csv(., "outputs/forest_change_data_all.csv") %>%
  mutate(perc_wc = (area_wc / area_ha) * 100) %>%
...
perc_plot <- 
  lc_change_l3_shp %>%
  mutate(class_01 = sapply(strsplit(change_class, " "), tail, 1),
         class_21 = sapply(strsplit(change_class, " "), head, 1)) %>%
  ggplot() +
  geom_sf(aes(geometry = geometry,
              fill = ifelse(sig == "non_sig", NA, perc_wc_slope),
              color = "")) +
  scale_fill_steps2(low = "#8E00A4",
                    mid = "white",
                    high = "#136600",
                    midpoint = 0, limits = c(-2.5, 2.5), na.value = "grey",
                    name = "Sen's Slope\n(%/year)",
...
# Save the combined plot
ragg::agg_png(filename = "outputs/objective_2/infilling_perc_l3_fig.png", scaling = 0.9,
...
area_plot <- 
  ...
  scale_fill_steps2(low = "#8E00A4",
                    mid = "white",
                    high = "#136600",
                    midpoint = 0, limits = c(-5000,25000), na.value = "grey",
                    name = "Sen's Slope\n(ha/year)",
                    n.breaks = 8, nice.breaks = TRUE, show.limits = F) +
  scale_color_manual(values  = NA, labels = "n.s.") +
  guides(fill  = guide_colorsteps(order = 1,
...
# Save the combined plot
ragg::agg_png(filename = "outputs/objective_2/infilling_area_l3_fig.png", scaling = 0.9,
              height = 7, width = 7, units = "in", res = 600)
print(area_plot)
invisible(dev.off())
```
...
ft_l3 <- lc_change_l3 %>%
  ...
  mutate(across(matches(c("area_ha", "area_wc", "area_change")) &
                  !matches(c("perc_wc_p_value", "perc_area_change")),
                ~ round(., 0))) %>%
  mutate(perc_area_change = round(perc_area_change, 1)) %>%
  mutate(across(where(is.numeric), ~ round(., 3))) %>%
  mutate(across(matches("p_value$"), ~ ifelse(. < 0.001, "<0.001", .))) %>%
  arrange(us_l3name) %>%
  write_csv("outputs/forest_change_data.csv") %>%
  write_csv("outputs/data/infilling_change_ft_l3.csv")
```
...
plots <- merged_ch %>%
  ...
  arrange(us_l3name) %>%
  mutate(change_class = str_wrap(change_class, width = 40)) %>%
  group_by(us_l3name) %>%
  group_split() %>%
...
plots %>%
  imap(~ ggsave(filename = paste0(str_replace_all(to_snake_case(.y), "[ /]", "_"), "_area_change.jpg"),
                plot = .x,
                device = "jpeg",
                path = "outputs/objective_2/infilling_linegraphs",
