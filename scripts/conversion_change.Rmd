...
```{r import_data_ch}
area_ch <- fread("data/nwood_forest/nlcd_area_nwood_forest.csv")
file_path_ch <- "data/nwood_forest/"
file_names_ch <- paste0("nlcd_rap_nwood_forest_", 2001:2021, ".csv")
...
total <- rbind(forest_total, type_total) %>%
  ...
  write_csv("data/nwood_forest_no_type.csv")
...
lc_change_ch <- merged_ch %>%
  ...
  mutate(across(where(is.numeric),
                ~ case_when(. >= 100 ~ round(., 0),
                            ...
                            . <= 1 & . >= -1 ~ round(., 3),
                            TRUE ~ round(., 0)))) %>%
  write_csv("outputs/data/conversion_change_ft_l1.csv")
```
...
ragg::agg_png(filename = "outputs/objective_2/conversion_perc_l1_fig.png", scaling = 0.9,
...
# Save the combined plot
ragg::agg_png(filename = "outputs/objective_2/conversion_area_l1_fig.png", scaling = 0.9,
              height = 3, width = 5, units = "in", res = 600)
print(area_plot)
invisible(dev.off())
```
...
sen_data_l3 <- merged_ch %>%
  ...
  summarise(area_wc = as.numeric(sum(area_wc)),
            area_ha = as.numeric(sum(area_ha)), .groups = "drop") %>%
  write_csv(., "outputs/nwood_forest_change_data_all.csv") %>%
  mutate(perc_wc = (area_wc / area_ha) * 100) %>%
...
# Save the combined plot
ragg::agg_png(filename = "outputs/objective_2/conversion_perc_l3_fig.png", scaling = 0.9,
...
# Save the combined plot
ragg::agg_png(filename = "outputs/objective_2/conversion_area_l3_fig.png", scaling = 0.9,
              height = 12, width = 7, units = "in", res = 600)
print(area_plot)
invisible(dev.off())
```
...
ft_l3 <- lc_change_l3 %>%
  ...
  arrange(us_l3name) %>%
  write_csv(., "outputs/nwood_forest_change_data.csv") %>%
  write_csv("outputs/data/conversion_change_ft_l3.csv")
```
...
plots <- merged_ch %>%
  ...
  arrange(us_l3name) %>%
  mutate(change_class = str_wrap(change_class, width = 20)) %>%
  group_by(us_l3name) %>%
  group_split() %>%
...
plots %>%
  imap(~ ggsave(filename = paste0(str_replace_all(to_snake_case(.y), "[ /]", "_"), "_area_change.jpg"),
                plot = .x,
                device = "jpeg",
                path = "outputs/objective_2/conversion_linegraphs",
